...
public class BookService {
    ...
    private AvailablityRequestRepo requestRepo;
    private UserRepo userRepo;
    private EmailService emailService;

    public BookService(BookRepo bookRepo, PublisherRepo publisherRepo, CategoryRepo categoryRepo,EntityMapper entityMapper, PublisherService publisherService, AvailablityRequestRepo requestRepo, UserRepo userRepo, EmailService emailService){
    ...
    public BookStoreDTO updateBook(int id, BookStoreDTO bookDTO) {
        ...
        if(matchingBook.getStock()<=0 && bookDTO.getStock()>0){
           List<BookAvailablityRequest> requests = requestRepo.findByBookNameAndNotifiedFalse(matchingBook.getBookName());
           emailService.sendAvailabilityEmail(requests);
           requestRepo.markAsNotified(matchingBook.getBookName());
        }
    ...
    public BookAvailablityRequest notifyUser(BookAvailablityRequest request) {
        BookStore book = bookRepo.findByBookNameContainingIgnoreCase(request.getBookName()).get(0);
        
        if(book.getStock()>0){
            throw new BusinessException("620","Book in stock");
        }

        request.setUserEmail(userRepo.findByUsername(request.getUserName()).getEmail());
